...
public class DJMapper : MonoBehaviour
{
    ...
    [Tooltip("Crossfader Settings")]
    public CrossfaderConfig crossfaderConfig = new CrossfaderConfig
    {
        ccNumber = 20,
        channel = MidiChannel.Ch1  // Supposons que le crossfader soit sur le canal 3
    };
    ...
    private Dictionary<string, float> lastCCValues = new Dictionary<string, float>();
    private Dictionary<string, bool> lastNoteStates = new Dictionary<string, bool>();
    #endregion
    ...
    public struct DJMidiConfig
    {
        ...
        public int pad3;
        public int pad4;
        public MidiChannel channel; // Ajout du canal MIDI
    }
    
    [Serializable]
    public struct CrossfaderConfig
    {
        public int ccNumber;
    ...
    private void Update()
    {
        ...
        CheckCC(deckAConfig.volumeFader, deckAConfig.channel, value => UpdateDeckVolume(0, value));
        CheckCC(deckBConfig.volumeFader, deckBConfig.channel, value => UpdateDeckVolume(1, value));
        CheckCC(deckAConfig.jogWheel, deckAConfig.channel, value => UpdateJog(0, value));
        CheckCC(deckBConfig.jogWheel, deckBConfig.channel, value => UpdateJog(1, value));
        CheckCC(crossfaderConfig.ccNumber, crossfaderConfig.channel, UpdateCrossfader);
        
        // EQs
        CheckCC(deckAConfig.bassEQ, deckAConfig.channel, value => UpdateEQ(0, 0, value));
        CheckCC(deckAConfig.midEQ, deckAConfig.channel, value => UpdateEQ(0, 1, value));
        CheckCC(deckAConfig.highEQ, deckAConfig.channel, value => UpdateEQ(0, 2, value));
        CheckCC(deckBConfig.bassEQ, deckBConfig.channel, value => UpdateEQ(1, 0, value));
        CheckCC(deckBConfig.midEQ, deckBConfig.channel, value => UpdateEQ(1, 1, value));
        CheckCC(deckBConfig.highEQ, deckBConfig.channel, value => UpdateEQ(1, 2, value));
        
        // Filtres
        CheckCC(deckAConfig.filter, deckAConfig.channel, value => UpdateFilter(0, value));
        CheckCC(deckBConfig.filter, deckBConfig.channel, value => UpdateFilter(1, value));
        
        // Vérification des boutons via polling (alternative aux délégués)
        CheckNote(deckAConfig.playPauseNote, deckAConfig.channel, pressed => UpdatePlayPause(0, pressed));
        CheckNote(deckAConfig.cueNote, deckAConfig.channel, pressed => UpdateCue(0, pressed));
        CheckNote(deckAConfig.syncNote, deckAConfig.channel, pressed => UpdateSync(0, pressed));
        CheckNote(deckBConfig.playPauseNote, deckBConfig.channel, pressed => UpdatePlayPause(1, pressed));
        CheckNote(deckBConfig.cueNote, deckBConfig.channel, pressed => UpdateCue(1, pressed));
        CheckNote(deckBConfig.syncNote, deckBConfig.channel, pressed => UpdateSync(1, pressed));
        
        // Vérification des pads
        for (int i = 0; i < padsPerDeck; i++)
        {
            CheckNote(deckAConfig.padBaseNote + i, deckAConfig.channel, pressed => UpdatePad(0, i, pressed));
            CheckNote(deckBConfig.padBaseNote + i, deckBConfig.channel, pressed => UpdatePad(1, i, pressed));
            
            // CheckNote(deckAConfig.pad1 + i, deckAConfig.channel, pressed => UpdatePad(1, i, pressed));
        }
    }
    
    #region Méthodes d'aide pour la vérification des contrôles
    private void CheckCC(int ccNumber, MidiChannel channel, Action<float> updateAction)
    {
        // Utiliser MidiMaster.GetKnob avec le canal spécifique si disponible
        // Si MidiJack ne supporte pas GetKnob avec canal, vous devrez adapter cette logique
        float currentValue = MidiMaster.GetKnob(channel, ccNumber);
    
        // Créer une clé unique qui combine canal et numéro CC
        string key = $"{channel}_{ccNumber}";
    
        // Si la valeur existe dans le dictionnaire et a changé significativement
        if (lastCCValues.ContainsKey(key))
        {
            if (Mathf.Abs(currentValue - lastCCValues[key]) > changeThreshold)
            {
                lastCCValues[key] = currentValue;
        ...
        else
        {
            ...
            lastCCValues.Add(key, currentValue);
            if (currentValue > 0)
            {
                updateAction(currentValue);
            }
    ...
    private void CheckNote(int noteNumber, MidiChannel channel, Action<bool> updateAction)
    {
        // Utiliser MidiMaster.GetKey avec le canal spécifique
        bool isPressed = MidiMaster.GetKey(channel, noteNumber) > 0;
    
        // Créer une clé unique qui combine canal et note
        string key = $"{channel}_{noteNumber}";
    
        if (lastNoteStates.ContainsKey(key))
        {
            if (lastNoteStates[key] != isPressed)
            {
                lastNoteStates[key] = isPressed;
                updateAction(isPressed);
            }
        }
        else
        {
            lastNoteStates.Add(key, isPressed);
            if (isPressed)
            {
                updateAction(isPressed);
            }
    ...
    {
        // Deck A - vérifier à la fois le canal et la note
        if (channel == deckAConfig.channel)
        {
        ...
        // Deck B - vérifier à la fois le canal et la note
        else if (channel == deckBConfig.channel)
        {
            if (note == deckBConfig.playPauseNote)
    ...
    private void HandleKnob(MidiChannel channel, int knobNumber, float knobValue)
    {
        ...
        {
            if (knobNumber == deckBConfig.volumeFader)
        ...
        // Crossfader - sur son propre canal
        else if (channel == crossfaderConfig.channel && knobNumber == crossfaderConfig.ccNumber)
        {
    ...
    public void SetMidiConfig(int deck, string controlName, int midiNumber)
    {
        ...
        {
            crossfaderConfig.ccNumber = midiNumber;
        }
